...
from src.chains.reflexes.voice_scout import run_voice_scout
from src.chains.reflexes.cyber_scout import run_cyber_scout
from src.chains.reflexes.vision_scout import run_vision_scout
from src.chains.reflexes.legal_brain import draft_legal_notice
...
async def node_scout_swarm(state: ChainReflexState) -> Dict[str, Any]:
    ...
    add_log(
        "SCOUT",
        f"TOP THREAT: {top_disruption.severity_level} at {top_disruption.location}",
        "alert",
    )
...
def node_legal_brain(state: ChainReflexState) -> Dict[str, Any]:
    ...
    if current_iteration > 0:
        add_log(
            "LEGAL_BRAIN",
            "Rewriting notice to comply with security policies...",
            "info",
        )
        time.sleep(0.5)
        ...
        draft = draft_legal_notice(
            disruption_data={
                ...
                "materials": disruption.affected_materials,
            },
    ...
    return {"drafted_legal_notice": draft, "iteration_count": current_iteration + 1}
...
def node_firewall(state: ChainReflexState) -> Dict[str, Any]:
    ...
    if "$50,000" in draft or "ransomware payouts" in draft.lower():
        add_log(
            "FIREWALL",
            "BREACH PREVENTED: Unauthorized financial commitment found!",
            "alert",
        )
...
workflow.add_conditional_edges(
    ...
    {"legal": "Legal", "auto_log": "AutoLog", "end": END},
)
