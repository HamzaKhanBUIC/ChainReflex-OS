...
KERNEL_DIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "rocm_kernel")
)
...
try:
    ...
    if os.path.exists(LIB_PATH):
        ...
        logger.warning(
            f"C++ Kernel not found at {LIB_PATH}. Switching to high-fidelity simulation."
        )
        parser_lib = None
except OSError:
    logger.error(
        "Failed to initialize hardware acceleration. Using ROCm simulation layer."
    )
    parser_lib = None
