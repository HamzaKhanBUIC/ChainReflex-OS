...
"analysis": "A concise 1-sentence explanation of the situation described in the transcript."
    ...
    try:
        ...
        if os.path.exists(audio_path):
            print("   -> Real audio file detected. Running Whisper-v3 Transcription...")
            with open(audio_path, "rb") as f:
                audio_data = f.read()
            # Note: Serverless Whisper can be slow or hit size limits
            whisper_result = client.automatic_speech_recognition(
                audio_data, model="openai/whisper-large-v3"
            )
            transcript = whisper_result.text
        else:
            print(
                f"   -> Audio file not found. Using simulation transcript for: {audio_path}"
            )
            transcript = mock_transcripts.get(
                audio_path,
                "Ground report: Unconfirmed reports of road blockades near the coastal facility.",
            )

        # Step 2: Analyze transcript with Llama 3
        print("   -> Analyzing transcript with Llama-3-8B-Instruct...")
        ...
        print(
            f"   [ALERT] {data.get('threat_type')} detected in audio at {data.get('location_context')}!"
        )
