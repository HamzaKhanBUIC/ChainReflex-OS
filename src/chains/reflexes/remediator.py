...
import logging
import httpx
from src.chains.router import get_full_context
...
async def generate_remediated_file(
    repository: str,
    file_path: str,
    ...
    if vllm_base and "localhost" not in vllm_base:
        ...
        logger.info(
            f"[{repository}] Swarm Iteration 1: AI Agent synthesizing fix via MI300X..."
        )
    else:
        # Fallback: Hugging Face Inference API
        api_url = "https://api-inference.huggingface.co/models/Qwen/Qwen2.5-Coder-32B-Instruct"
        logger.info(
            f"[{repository}] Swarm Iteration 1: AI Agent synthesizing fix via Cloud fail-safe..."
        )
    ...
    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if "```" in patched_content:
                ...
                if patched_content.startswith("python") or patched_content.startswith(
                    "py"
                ):
