...
from main import handle_autonomous_response
from agents.remediator import generate_remediated_file, generate_pr_body
from core.github_integration import open_remediation_pr
from core.telemetry import get_logs, add_log
from core.notifications import send_discord_alert
...
async def trigger_autonomous_response(signal: IngestSignal):
    ...
    add_log("SYSTEM", f"Incoming {signal.threat_vector.upper()} vector detected.", "warning")
...
async def receive_vulnerability_alert(alert: VulnerabilityAlert, background_tasks: BackgroundTasks):
    """
...
async def process_remediation_swarm(alert: VulnerabilityAlert):
    ...
    try:
        ...
        if not patched_code:
            logger.error(f"[{alert.repository}] Swarm failed to synthesize a valid patch.")
            add_log("ORACLE", "Patch synthesis failed. Human intervention required.", "alert")
            return
        ...
        pr_body = await generate_pr_body(alert.repository, alert.file_path, report_for_drafter)
...
async def legacy_remediate_bridge(alert: VulnerabilityAlert, background_tasks: BackgroundTasks):
